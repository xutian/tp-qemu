...
from . import storage_volume
from . import virt_encryption
from .backend import directory, direct_iscsi, nfs, rbd, gluster
...
class StoragePoolAdmin(object):
    ...
    def acquire_volume(cls, volume):
        def _acquire_volume(vol):
            if vol.is_allocated:
                return
            backing_store = vol.format.get_param("data_file")
            if backing_store:
                _acquire_volume(backing_store)
